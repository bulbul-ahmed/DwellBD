import { useState, useEffect } from 'react'
import { Calendar, MapPin, Clock, CheckCircle, AlertCircle, XCircle, Trash2 } from 'lucide-react'
import { Link } from 'react-router-dom'
import toast from 'react-hot-toast'
import Button from '../components/ui/Button'
import { getTenantVisitRequests, cancelVisitRequest, formatDate, formatTime, formatVisitStatus, getStatusColor } from '../api/visitApi'

interface VisitRequest {
  id: string
  propertyId: string
  tenantId: string
  ownerId: string
  requestedDate: string
  requestedTime: string
  confirmedDate?: string
  confirmedTime?: string
  suggestedDate?: string
  suggestedTime?: string
  tenantNote?: string
  ownerNote?: string
  rejectionReason?: string
  status: 'PENDING' | 'CONFIRMED' | 'PENDING_ALTERNATIVE' | 'REJECTED' | 'CANCELLED' | 'COMPLETED'
  createdAt: string
  updatedAt: string
  completedAt?: string
  property?: {
    id: string
    title: string
    area: string
    images: string[]
    coverImage?: string
  }
  owner?: {
    id: string
    firstName: string
    lastName: string
    email: string
    phone?: string
  }
}

const VisitsPage = () => {
  const [visits, setVisits] = useState<VisitRequest[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [filterStatus, setFilterStatus] = useState<'ALL' | 'PENDING' | 'CONFIRMED' | 'PENDING_ALTERNATIVE' | 'REJECTED' | 'CANCELLED' | 'COMPLETED'>('ALL')
  const [cancellingId, setCancellingId] = useState<string | null>(null)

  useEffect(() => {
    loadVisits()
  }, [])

  const loadVisits = async () => {
    try {
      setIsLoading(true)
      const visitsList = await getTenantVisitRequests()
      setVisits(visitsList)
    } catch (error) {
      console.error('Error loading visits:', error)
      toast.error('Failed to load visit requests')
    } finally {
      setIsLoading(false)
    }
  }

  const getFilteredVisits = () => {
    if (filterStatus === 'ALL') return visits
    return visits.filter((v) => v.status === filterStatus)
  }

  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'PENDING':
        return <AlertCircle className="h-5 w-5 text-yellow-600" />
      case 'CONFIRMED':
        return <CheckCircle className="h-5 w-5 text-blue-600" />
      case 'PENDING_ALTERNATIVE':
        return <AlertCircle className="h-5 w-5 text-orange-600" />
      case 'REJECTED':
        return <XCircle className="h-5 w-5 text-red-600" />
      case 'COMPLETED':
        return <CheckCircle className="h-5 w-5 text-green-600" />
      case 'CANCELLED':
        return <XCircle className="h-5 w-5 text-gray-600" />
      default:
        return null
    }
  }

  const getStats = () => {
    const total = visits.length
    const pending = visits.filter((v) => v.status === 'PENDING').length
    const confirmed = visits.filter((v) => v.status === 'CONFIRMED').length
    const completed = visits.filter((v) => v.status === 'COMPLETED').length
    const rejected = visits.filter((v) => v.status === 'REJECTED').length

    return { total, pending, confirmed, completed, rejected }
  }

  const handleCancelVisit = async (visitId: string) => {
    if (!window.confirm('Are you sure you want to cancel this visit request?')) return

    try {
      setCancellingId(visitId)
      await cancelVisitRequest(visitId)
      setVisits((prev) =>
        prev.map((v) => (v.id === visitId ? { ...v, status: 'CANCELLED' } : v))
      )
      toast.success('Visit request cancelled')
    } catch (error) {
      console.error('Error cancelling visit:', error)
      toast.error('Failed to cancel visit request')
    } finally {
      setCancellingId(null)
    }
  }

  const stats = getStats()
  const filteredVisits = getFilteredVisits()

  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          <div className="h-12 w-12 animate-spin rounded-full border-4 border-gray-200 border-t-primary-600"></div>
          <p className="mt-4 text-gray-600">Loading your visit requests...</p>
        </div>
      </div>
    )
  }

  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-6xl">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">My Property Visits</h1>
          <p className="mt-2 text-gray-600">
            Track and manage your property visit requests
          </p>
        </div>

        {/* Stats Dashboard */}
        {visits.length > 0 && (
          <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <div className="rounded-lg bg-white p-6 shadow">
              <p className="text-sm font-medium text-gray-600">Total Requests</p>
              <p className="mt-2 text-3xl font-bold text-gray-900">{stats.total}</p>
            </div>
            <div className="rounded-lg bg-yellow-50 p-6 shadow">
              <p className="text-sm font-medium text-yellow-800">Pending</p>
              <p className="mt-2 text-3xl font-bold text-yellow-900">{stats.pending}</p>
            </div>
            <div className="rounded-lg bg-blue-50 p-6 shadow">
              <p className="text-sm font-medium text-blue-800">Confirmed</p>
              <p className="mt-2 text-3xl font-bold text-blue-900">{stats.confirmed}</p>
            </div>
            <div className="rounded-lg bg-green-50 p-6 shadow">
              <p className="text-sm font-medium text-green-800">Completed</p>
              <p className="mt-2 text-3xl font-bold text-green-900">{stats.completed}</p>
            </div>
            <div className="rounded-lg bg-red-50 p-6 shadow">
              <p className="text-sm font-medium text-red-800">Rejected</p>
              <p className="mt-2 text-3xl font-bold text-red-900">{stats.rejected}</p>
            </div>
          </div>
        )}

        {/* Filter */}
        {visits.length > 0 && (
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <select
              value={filterStatus}
              onChange={(e) => setFilterStatus(e.target.value as any)}
              className="rounded-lg border border-gray-300 px-4 py-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary-600"
            >
              <option value="ALL">All Visits</option>
              <option value="PENDING">Awaiting Confirmation</option>
              <option value="CONFIRMED">Confirmed</option>
              <option value="PENDING_ALTERNATIVE">Alternative Suggested</option>
              <option value="COMPLETED">Completed</option>
              <option value="REJECTED">Rejected</option>
              <option value="CANCELLED">Cancelled</option>
            </select>
            <p className="text-sm text-gray-600">Showing {filteredVisits.length} visit request(s)</p>
          </div>
        )}

        {/* Empty State */}
        {filteredVisits.length === 0 ? (
          <div className="rounded-lg bg-white p-12 text-center shadow">
            <Calendar className="mx-auto h-12 w-12 text-gray-400" />
            <h3 className="mt-4 text-lg font-medium text-gray-900">
              {visits.length === 0 ? 'No visit requests yet' : 'No visits match your filter'}
            </h3>
            <p className="mt-2 text-gray-600">
              {visits.length === 0
                ? 'Start by browsing properties and requesting a visit from the owner'
                : 'Try changing your filter to see other visit requests'}
            </p>
            {visits.length === 0 && (
              <Link to="/properties">
                <Button className="mt-6 bg-primary-600 hover:bg-primary-700">
                  Browse Properties
                </Button>
              </Link>
            )}
          </div>
        ) : (
          <div className="space-y-4">
            {filteredVisits.map((visit) => (
              <div
                key={visit.id}
                className="rounded-lg bg-white shadow transition-shadow hover:shadow-md"
              >
                <div className="flex flex-col gap-4 p-6 sm:flex-row">
                  {/* Property Image */}
                  <div className="flex-shrink-0">
                    <Link to={`/properties/${visit.propertyId}`}>
                      <img
                        src={visit.property?.coverImage || visit.property?.images?.[0] || 'https://via.placeholder.com/200x150?text=No+Image'}
                        alt={visit.property?.title}
                        className="h-40 w-40 rounded-lg object-cover hover:opacity-80 transition-opacity cursor-pointer"
                      />
                    </Link>
                  </div>

                  {/* Content */}
                  <div className="flex-1">
                    {/* Header */}
                    <div className="flex items-start justify-between">
                      <div>
                        <Link to={`/properties/${visit.propertyId}`}>
                          <h3 className="text-lg font-semibold text-gray-900 hover:text-primary-600">
                            {visit.property?.title}
                          </h3>
                        </Link>
                        <div className="mt-1 flex items-center text-sm text-gray-600">
                          <MapPin className="mr-1 h-4 w-4" />
                          {visit.property?.area}
                        </div>
                      </div>
                      <div className={`flex items-center gap-2 rounded-full px-3 py-1 ${getStatusColor(visit.status)}`}>
                        {getStatusIcon(visit.status)}
                        <span className="text-sm font-medium">{formatVisitStatus(visit.status)}</span>
                      </div>
                    </div>

                    {/* Details Grid */}
                    <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      {/* Requested Date/Time */}
                      <div>
                        <p className="text-xs font-medium text-gray-500">REQUESTED</p>
                        <div className="mt-1 flex items-center text-sm text-gray-900">
                          <Calendar className="mr-2 h-4 w-4" />
                          {formatDate(visit.requestedDate)}
                        </div>
                        <div className="mt-1 flex items-center text-sm text-gray-900">
                          <Clock className="mr-2 h-4 w-4" />
                          {formatTime(visit.requestedTime)}
                        </div>
                      </div>

                      {/* Confirmed Date/Time (if confirmed) */}
                      {visit.confirmedDate && (
                        <div>
                          <p className="text-xs font-medium text-gray-500">CONFIRMED</p>
                          <div className="mt-1 flex items-center text-sm text-green-600 font-medium">
                            <Calendar className="mr-2 h-4 w-4" />
                            {formatDate(visit.confirmedDate)}
                          </div>
                          <div className="mt-1 flex items-center text-sm text-green-600 font-medium">
                            <Clock className="mr-2 h-4 w-4" />
                            {formatTime(visit.confirmedTime!)}
                          </div>
                        </div>
                      )}

                      {/* Suggested Alternative (if pending alternative) */}
                      {visit.suggestedDate && (
                        <div>
                          <p className="text-xs font-medium text-gray-500">OWNER SUGGESTED</p>
                          <div className="mt-1 flex items-center text-sm text-orange-600 font-medium">
                            <Calendar className="mr-2 h-4 w-4" />
                            {formatDate(visit.suggestedDate)}
                          </div>
                          <div className="mt-1 flex items-center text-sm text-orange-600 font-medium">
                            <Clock className="mr-2 h-4 w-4" />
                            {formatTime(visit.suggestedTime!)}
                          </div>
                        </div>
                      )}

                      {/* Owner Contact */}
                      {visit.owner && (
                        <div>
                          <p className="text-xs font-medium text-gray-500">OWNER</p>
                          <p className="mt-1 text-sm font-medium text-gray-900">
                            {visit.owner.firstName} {visit.owner.lastName}
                          </p>
                          <p className="text-sm text-gray-600">{visit.owner.email}</p>
                          {visit.owner.phone && (
                            <p className="text-sm text-gray-600">{visit.owner.phone}</p>
                          )}
                        </div>
                      )}
                    </div>

                    {/* Notes Section */}
                    {(visit.tenantNote || visit.ownerNote || visit.rejectionReason) && (
                      <div className="mt-4 space-y-2 border-t border-gray-100 pt-4">
                        {visit.tenantNote && (
                          <div className="text-sm">
                            <p className="font-medium text-gray-700">Your Message:</p>
                            <p className="mt-1 text-gray-600">{visit.tenantNote}</p>
                          </div>
                        )}
                        {visit.ownerNote && (
                          <div className="text-sm">
                            <p className="font-medium text-gray-700">Owner Message:</p>
                            <p className="mt-1 text-gray-600">{visit.ownerNote}</p>
                          </div>
                        )}
                        {visit.rejectionReason && (
                          <div className="text-sm">
                            <p className="font-medium text-red-700">Rejection Reason:</p>
                            <p className="mt-1 text-red-600">{visit.rejectionReason || 'No reason provided'}</p>
                          </div>
                        )}
                      </div>
                    )}

                    {/* Actions */}
                    <div className="mt-4 flex flex-wrap gap-2 border-t border-gray-100 pt-4">
                      <Link to={`/properties/${visit.propertyId}`}>
                        <Button
                          variant="secondary"
                          className="flex items-center gap-2"
                        >
                          <MapPin className="h-4 w-4" />
                          View Property
                        </Button>
                      </Link>

                      {(visit.status === 'PENDING' || visit.status === 'PENDING_ALTERNATIVE') && (
                        <button
                          onClick={() => handleCancelVisit(visit.id)}
                          disabled={cancellingId === visit.id}
                          className="inline-flex items-center gap-2 rounded-lg border border-red-300 px-3 py-2 text-sm font-medium text-red-700 transition-colors hover:bg-red-50 disabled:opacity-50"
                        >
                          <Trash2 className="h-4 w-4" />
                          {cancellingId === visit.id ? 'Cancelling...' : 'Cancel Request'}
                        </button>
                      )}

                      {visit.rejectionReason && (
                        <Link to="/properties">
                          <Button className="flex items-center gap-2 bg-primary-600 hover:bg-primary-700">
                            <Calendar className="h-4 w-4" />
                            Request Another Property
                          </Button>
                        </Link>
                      )}
                    </div>
                  </div>
                </div>
              </div>
            ))}
          </div>
        )}
      </div>
    </div>
  )
}

export default VisitsPage
